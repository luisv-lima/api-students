...
import (
	...
	"github.com/luisv-lima/api-students/schemas"
...
func (api *API) getStudents(c echo.Context) error {
	...
	if active != "" {
		...
		if err != nil {
			log.Error().Err(err).Msgf("[api] failed to get filtered students")
...
func (api *API) getStudent(c echo.Context) error {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return c.String(http.StatusNotFound, "Student no found")
	}
